...
const PlaceCircleLabel = observer(({ className, placeCircle, theme, device }: PlaceCircleLabelProps) => {
  ...
  const drawLabel = useCallback(() => {
    ...
    ref.current!.setAttributeNS('http://www.w3.org/1999/xlink', 'href', canvas.toDataURL());
    ref.current!.setAttribute('width', String(width * 0.5));
    ref.current!.setAttribute('height', String(height * 0.5));
    ref.current!.style[DomUtil.TRANSFORM] = `translateX(${Math.round(width * -0.25)}px)`;
  }, [theme, placeCircle, device, ref.current]);

  useEffect(() => {
    return autorun(drawLabel);
  }, [drawLabel]);
...
/*@observer
class PlaceCircleLabel extends Component<PlaceCircleLabelProps> {
  private readonly ref: RefObject<SVGImageElement>;
  private labelCanvas?: HTMLCanvasElement;
  private ctx?: CanvasRenderingContext2D | null;

  constructor(props: PlaceCircleLabelProps) {
    super(props);
